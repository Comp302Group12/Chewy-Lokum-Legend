...
public class SimpleGraphicsDrawingAdapter extends DrawingAdapter {
	...
	public void draw(Graphics g, NormalLokum lokum) {
		...
		g.fillRoundRect(lokum.getX(), lokum.getY(), lokum.getWidth(), lokum.getHeight(), lokum.getWidth()
				/ ROUND_RECT_ARC_CONS_FOR_LOKUMS, lokum.getHeight() / ROUND_RECT_ARC_CONS_FOR_LOKUMS);
	}
	...
	public void draw(Graphics g, StripedLokum lokum) {
		...
		if (lokum.isHorizontal()) {
			...
			g.fillRect(lokum.getX(), lokum.getY() + heightOfLokumPart, lokum.getWidth(),
					heightOfLokumPart);
			g.fillRect(lokum.getX(), lokum.getY() + 3 * heightOfLokumPart, lokum.getWidth(),
					heightOfLokumPart);
		} else {
			...
			g.fillRoundRect(lokum.getX(), lokum.getY(), lokum.getWidth(), lokum.getHeight(), lokum.getWidth()
					/ ROUND_RECT_ARC_CONS_FOR_LOKUMS, lokum.getHeight() / ROUND_RECT_ARC_CONS_FOR_LOKUMS);
			g.setColor(Color.WHITE);
			g.fillRect(lokum.getX() + widthOfLokumPart, lokum.getY(), widthOfLokumPart,
					lokum.getHeight());
			g.fillRect(lokum.getX() + 3 * widthOfLokumPart, lokum.getY(), widthOfLokumPart,
					lokum.getHeight());
		}
	}
	...
	public void draw(Graphics g, WrappedLokum lokum) {
		...
		g.fillRoundRect(lokum.getX(), lokum.getY(), lokum.getWidth(), lokum.getHeight(), lokum.getWidth()
				/ ROUND_RECT_ARC_CONS_FOR_LOKUMS, lokum.getHeight() / ROUND_RECT_ARC_CONS_FOR_LOKUMS);
		g.setColor(lokum.getColor());
		g.fillOval((lokum.getX() + (lokum.getWidth() - lokum.getWidth() * 3 / 5) / 2),
	...
	public void draw(Graphics g, ColorBombLokum lokum) {
		...
		for (int i = 0; i < Lokum.lokumColors.length; i++) {
			int widthOfLokumPart = (lokum.getWidth() / Lokum.lokumColors.length) * (Lokum.lokumColors.length - i );
			int heightOfLokumPart = (lokum.getHeight() / Lokum.lokumColors.length) * (Lokum.lokumColors.length - i);
			g.setColor(Lokum.lokumColors[i]);
			g.fillRoundRect((lokum.getX() + (lokum.getWidth() - widthOfLokumPart) / 2),
	...
	public void draw(Graphics g, Board board) {
		...
		for (int i = 0; i <= board.getNumOfLokumsInAColumn(); i++) {
			g.drawLine(0, i * board.getHeight() / board.getNumOfLokumsInAColumn(), board.getWidth(), i	* board.getHeight() / board.getNumOfLokumsInAColumn());
		}
		for (int i = 0; i <= board.getNumOfLokumsInARow(); i++) {
			g.drawLine(i * board.getWidth() / board.getNumOfLokumsInARow(), 0, i * board.getWidth()
					/ board.getNumOfLokumsInARow(), board.getHeight());
		}
		//drawing lokums
		for(int i=0; i<board.getNumOfLokumsInAColumn(); i++){
